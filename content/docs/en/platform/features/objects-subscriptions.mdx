...
<Callout type="info">
  **Note on variables**: Because objects are resolved and deduped to raw subscriber profiles at
  trigger time, object-specific properties are not automatically available inside template variable
  contexts. To display object details (like a project's name or status) in your notifications, pass
  them in the trigger request's `data` payload.
</Callout>
